...
let gensym _ = incr c ; "'"^(String.make 1 (char_of_int(int_of_char 'a' + !c - 1)))
...
let rec type_of (env : env) (e : expr) : (typ * env) =
  ...
  | ECar(e1) ->
     let type_e1, _ = type_of env e1 in
...
let rec valeur_de = function
  | TVariable({valeur = Connue ty1; niveau = 0} as var) -> (* TODO *)
