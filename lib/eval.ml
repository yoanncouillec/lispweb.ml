...
and js_of_scheme e =
  ...
  | ELambda(posparams, _, e1) -> JsFunction(posparams ,js_of_scheme e1)
...
and eval (e:expr) (genv:env) (env:env) (denv:env) (mem:mem) (cont:cont) =
  ...
  | EBegin (expression::rest) ->
     eval expression genv env denv mem 
       (fun _ genv' mem' -> eval (EBegin (rest)) genv' env denv mem' cont)
  ...
  | EUnQuote (_) -> failwith "eval EUnQuote: must be inside a quasiquote"
